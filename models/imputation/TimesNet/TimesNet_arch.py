...
class TimesNet(nn.Module):
    ...
    def __init__(self, model_args):
        ...
        self.model = nn.ModuleList([TimesBlock(model_args["seq_len"], model_args["pred_len"], model_args["top_k"], model_args["d_model"], model_args["d_ff"], model_args["num_kernels"])
                                    for _ in range(model_args["e_layers"])])
        self.enc_embedding = DataEmbedding_wo_pos(model_args["enc_in"], model_args["d_model"], self.time_of_day_size,self.day_of_week_size,self.day_of_month_size,
                                                  self.day_of_year_size, model_args["embed"], model_args["num_time_features"], model_args["dropout"])
        self.layer = model_args["e_layers"]
        self.layer_norm = nn.LayerNorm(model_args["d_model"])
        self.predict_linear = nn.Linear(self.seq_len, self.pred_len + self.seq_len)
        self.projection = nn.Linear(model_args["d_model"], model_args["c_out"], bias=True)
